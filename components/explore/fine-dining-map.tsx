...
import { FINE_DINING_MAP_PIN_COLOR } from "@/lib/design/fine-dining-map";
import { getFineDiningForMap, getFineDiningMapCopy } from "@/lib/i18n/messages/explore-fine-dining-guide";
  ...
  return `
    <div class="market-pin" role="img" aria-label="${escapeHtml(placeLabel)}">
      <div class="market-pin__head" style="background-color:${color}">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" aria-hidden="true">
...
export function FineDiningMap() {
  const { language } = useLanguage();
  const venues = useMemo(() => getFineDiningForMap(language), [language]);
  const mapCopy = useMemo(() => getFineDiningMapCopy(language), [language]);
  ...
  useEffect(() => {
    ...
    venues.forEach((venue) => {
      const icon = Leaflet.divIcon({
        ...
        html: createPinIcon(venue.pin.shortLabel, venue.name, FINE_DINING_MAP_PIN_COLOR),
        iconSize: [52, 62],
        iconAnchor: [26, 62],
        popupAnchor: [0, -62],
      });
  ...
  return (
    <section
      ...
      aria-labelledby="fine-dining-map-title"
    >
      <div className="border-b border-[#e7e8e9] px-5 py-4 sm:px-6">
        <h3 id="fine-dining-map-title" className="text-xl font-bold text-[#191c1d] md:text-2xl">
          {title}
        </h3>
